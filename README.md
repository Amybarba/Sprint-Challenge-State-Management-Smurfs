...
It simplifies state management so that many components can utilize it without having to pass 
it through all parts. It does hinder components so that they are not reusable.

- [X] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
actions are what changes the state and need to be used with a dispatch call.
reducers are the part that process the actions
store is what holds the enire redux tree, kinda of like wrapping.

- [X] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is global and held in store, the entire app can hook into it.
Component state is local, so only within your component and its children can it be accessed.

- [X] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
"Redux Thunk is a middleware created by Dan Abramov, that provides the ability to handle asynchronous operations inside our Action Creators." - training kit
So now that redux was created it had a flaw of synchonicity, redux thunk was created to handle
asynchronous items before our action creators.

- [X] What is your favorite state management system you've learned and this sprint? Please explain why!
React's component state because I find I think I understandt it better.
